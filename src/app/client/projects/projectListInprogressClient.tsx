...
export default function ProjectListInprogressClient() {
    ...
    const handleProjects = checkFilter || checkIncompleteFilter ? filteredInprogressProjects : inprogressProjects;
                ...
                return (
                    <div className={`flex items-center ${showInprogressList ? "flex" : "hidden"}`} key={data.id}>
                        <div className="w-[50%] border-b flex items-center h-[42px] cursor-pointer">
                            ...
                            <Link href={`/tasks/${paramsId}/${data.id}`} className=" w-full h-full flex items-center hover:bg-[#F9F8F8] pl-1">
